...
$tsqlid = "select COUNT(*)+1 AS COUNT from USERS ";
...
if ($stmt3 === false) {
    ...
    $redirectFrom = isset($_POST['redirectFrom']) ? $_POST['redirectFrom'] : 'web';
    // Thành công, hiển thị thông báo pop-up và chuyển hướng về trang chủ
    
    echo "<script>
            alert('Tạo tài khoản thành công! Bạn sẽ được chuyển hướng về trang chủ.');
            window.location.href = 'web.php'; // Chuyển hướng về trang chủ
          </script>";
}
}
?> 
...
<body>
    ...
    <form method="POST" action="register.php"> <!-- Thay đổi action nếu cần -->
